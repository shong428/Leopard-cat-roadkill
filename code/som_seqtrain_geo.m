...
%SOM_SEQTRAIN_GEO  Use sequential algorithm to train the GEOGRAPHICAL Self-Organizing Map.
%
% [sM,sT] = som_seqtrain(sM, D, [[argID,] value, ...])
% 
%  GEOGRAPHICAL SOM options:
%  (ALL geographical variants assume geographical coordinates are cartesian and stores in
%   in the first two components of the pattern vector)
%
%   'geo_match'    (scalar)    Defines the radius of the geographical BMU that should be
%                              searched for the final BMU. A radius of ZERO implies that
%                              only geo-coordinates are used, thus originating a Hypermap.
%                              A radius of 1 or greater with originate a spatial-kangas map.
%   'geo_update'   (string)    Defines which components should be updated:
%                               'nongeo' - Updata only non-geographic components (DEFAULT)
%                               'all' - all components (including GEO) will be updated
%                               
%
%
...
% MODIFIED by V.Lobo, 04/04/06
    case 'geo_match',                   % Defines the RADIUS of the GEOgraphic BMU that must be
        i=i+1;                          % searched to find the true BMU
        geo_match_radius=varargin{i};
    
    case 'geo_update',                  % Defines if we should update 'all' components, or only 
...
% V.2 by V.Lobo, 04/04/06 (yy/mm/dd)
  ...
  geocoord=known-known;                  % build a 0-logical vector with the same size as x 
  geocoord(1)=1;                         % "activate" the geographical coordinates
  geocoord(2)=1;
  geocoord=logical(geocoord);
                                                 % Find the BMU considering ONLY the GEO coordinates
  Dx = M(:,geocoord) - x(mu_x_1,geocoord);       % each map unit minus the vector
  [qerr bmu_initial] = min((Dx.^2)*mask([1 2])); % minimum distance(^2) and the BMU_initial
  ...
  if geo_match_radius==0,
      ...
      non_searchable_units=find(Ud(:,bmu_initial)>geo_match_radius);
                                             ...
                                             % NOW, find the distances considering all components
      Dx = M(:,known) - x(mu_x_1,known);     % each map unit minus the vector
      tmp=(Dx.^2)*mask(known);
      tmp(non_searchable_units)=inf;         % Exclude units that are not in the KANGAS Neighborhood
